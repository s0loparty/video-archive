		...
		<div class="vpage">
			<div :class="['vpage__wrap', { 'vpage__wrap--col-3': similarVIdeos.length }]">
				...
				<div class="vpage__history">
					...
					<div
						v-else
						class="vpage__history-container"
					>
				...
				<div
					v-if="similarVIdeos.length"
					class="vpage__history vpage__similars"
				>
					<h3 class="vpage__history-title">
						<svg
							xmlns="http://www.w3.org/2000/svg"
...
const goNextVideo = () => {
	...
	if (!item) {
		const _temp_item = similarVIdeos.value.concat(
			historyVideos.filter((i) => {
...
const historyVideos = history
	.map((item) => allVideos.value.find((v) => v.id === item))
	.filter(Boolean)
	.reverse();
...
const similarVIdeos = computed(() => {
	return allVideos.value.filter(
		(v) =>
			v.title.indexOf(video.value.title.split(' ')[0]) !== -1 &&
			v.id !== video.value.id
	);
});
...
const nextVideo = computed(() => {
	return allVideos.value.filter((v) => !history.includes(v.id))[0];
});
</script>
